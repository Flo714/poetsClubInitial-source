...
<template>    
    ...
    <div class="hidden" id="addPoem">
      ...
      <label>Poem's title</label><br>
        ...
        <input type="checkbox" id="checkbox" v-model="hidden" checked/>
  ...
  methods: {  
    ...
    async createPoem(){
      var res;
        ...
        if(document.getElementById("checkbox").checked) this.hidden=true
        else this.hidden=false
        alert(this.hidden + " "+ this.email + " "+ this.title + " " +this.content + " " +res) 
        await supabase
            .from('poems')
            .insert([
